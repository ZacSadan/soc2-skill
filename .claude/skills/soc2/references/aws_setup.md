...
IAM, S3 (`list_buckets`), the root account summary, CloudTrail, and Resource Explorer are global/account-wide and always scanned once regardless of `aws.regions`. EC2 (security groups, key pairs, EBS default encryption, VPC Flow Logs), API Gateway, Security Hub, GuardDuty, Access Analyzer, AWS Config Recorder, KMS, and RDS are region-scoped - the scanner loops each of those checks across every region listed in `config/soc2.config.yaml`'s `aws.regions`. Leave it empty (the default) to auto-discover every region enabled for the account via `ec2:DescribeRegions`, or pin an explicit list to scan a subset faster/cheaper. Each region-scoped resource's report row shows its `Region`, and its snapshot `id` is prefixed with the region so the diff engine never conflates the same-shaped resource across two regions. CloudTrail is deliberately *not* looped even though it's technically per-region-visible - it already models multi-region coverage itself via `IsMultiRegionTrail`, so a single call (against the default region) sees every trail without the risk of a per-region loop falsely flagging "no trail" in a region that's actually covered by an existing multi-region trail.
...
`scan_iam_policy_bindings` no longer relies on policy *names* alone: the 3 AWS built-in full-access managed policies (`AdministratorAccess`, `PowerUserAccess`, `IAMFullAccess`) are still matched by name (their content is fixed), but every inline policy and every customer-managed attached policy now has its actual JSON document fetched (`iam:GetUserPolicy`/`GetRolePolicy`/`GetGroupPolicy` for inline, `iam:GetPolicy`+`GetPolicyVersion` for customer-managed - all already covered by `SecurityAudit`) and content-scanned via `_policy_grants_full_access` for an `"Action": "*"` + `"Resource": "*"` Allow statement. An operator-named policy like `"Menahel"` that grants full access is now flagged `high` regardless of what it's called - this used to always show as `info`.
